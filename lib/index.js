...
var initRoutes = function(socket, io) {
  var setRoute = function(key, callbacks) {
      let _callbacks = callbacks.splice(0);
    return socket.on(key, function(data, respond) {
      ...
      for (let i=callbacks.length-1;i>0;i--){
          _callbacks[i-1] = function(__req, __res, __next){
              return args[i-1](request, response, callbacks[i]);
          }
      }
      return _callbacks[0](request, response);
    });
  };
